...
This version is adapted for the standardized timeseries columns:

timeseries_data columns:
['time(s)', 'voltage(V)', 'current(A)', 'temperature(℃)',
 'charge_capacity(Ah)', 'SOC']

constant_metadata columns:
['rate_capacity (Ah)', 'cycle', 'initial_capacity (Ah)', 'DR', 'SOH']

Important:
The raw timeseries column names are changed, but the internal sample
matrix order is intentionally kept consistent with the original code.
"""
...
# Metadata column names are unchanged
...
def extract_soc_samples(
    ...
    Internal SOC sample order is fixed as:

    sample[:, 0] = SOC
    sample[:, 1] = charge_capacity(Ah)
    sample[:, 2] = voltage(V)
    sample[:, 3] = SOH
    sample[:, 4] = current(A) / rate_capacity(Ah)
    sample[:, 5] = temperature(℃) / 60
    sample[:, 6] = rate_capacity(Ah)

    Therefore, data_load_soc() can still use:
    input  = sample[:, [1, 2]]
    label  = [sample[-1, 0], sample[0, 6], mean(sample[:, 4]), mean(sample[:, 5])]
    """
    ...
    for start in starts:
        ...
        # Keep local charge capacity starting from zero in each SOC window
        # sample[:, 1] is charge_capacity(Ah)
        window[:, 1] -= window[0, 1]
...
def extract_soh_samples(
    ...
    Internal SOH sample order is fixed as:

    sample[:, 0]  = SOH
    sample[:, 1]  = delta_q
    sample[:, 2]  = charge_capacity(Ah)
    sample[:, 3]  = time(s)
    sample[:, 4]  = dQ
    sample[:, 5]  = current(A) / rate_capacity(Ah)
    sample[:, 6]  = temperature(℃) / 60
    sample[:, 7]  = voltage(V)
    sample[:, 8]  = cycle
    sample[:, 9]  = DR
    sample[:, 10] = initial_capacity(Ah)
    sample[:, 11] = rate_capacity(Ah)

    Therefore, data_load_soh() can still use:
    input = sample[:, [2, 3, 4, 7]]
    """
    ...
    # Normalize charge capacity and time within the interpolated voltage segment
    ...
    # dQ is recalculated from charge capacity.
    # The original dQ_dV column is no longer required.
...
def data_load_soc(samples):
    ...
    sample[:, 3] = SOH
    sample[:, 4] = C-rate
    sample[:, 5] = normalized temperature
    sample[:, 6] = rate capacity
    """
...
def data_load_soh(samples):
    ...
    sample[:, 4]  = dQ
    sample[:, 5]  = C-rate
    sample[:, 6]  = normalized temperature
    sample[:, 7]  = voltage(V)
    sample[:, 8]  = cycle
    sample[:, 9]  = DR
    sample[:, 10] = initial capacity
    sample[:, 11] = rate capacity
    """
